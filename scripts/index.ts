...
const addTaskBtn = Helper.getElementById<HTMLButtonElement>("addTaskBtn");
addTaskBtn.addEventListener("click", addTask);
const titleInput = Helper.getElementById<HTMLInputElement>("taskTitle");
const descriptionInput =
  Helper.getElementById<HTMLTextAreaElement>("taskDescription");
const prioritySelect = Helper.getElementById<HTMLSelectElement>("taskPriority");
const dueDateInput = Helper.getElementById<HTMLInputElement>("taskDueDate");
const todoContainer = Helper.getElementById<HTMLDivElement>("todo-container");
const progressContainer =
  Helper.getElementById<HTMLDivElement>("progress-container");
const completedContainer = Helper.getElementById<HTMLDivElement>(
  "completed-container",
);
...
function addTask() {
  ...
  const priority = prioritySelect.value as keyof typeof TaskPriority;
  ...
  if (isEditMode && editingTaskId) {
    ...
    Helper.addToStorageList<Task>("tasks", newTask);
  }
  clearForm();
  closeModal();
  resetEditMode();
...
function renderTaskActions(taskStatus: TaskStatus, taskId: string): string {
...
function renderTasks() {
  const tasks = Helper.getFromStorage<Task[]>("tasks") || [];
  if (tasks.length === 0) {
    return;
  }
...
function renderEmptyStates(tasks: Task[]) {
  ...
  if (
    tasks.filter((task) => task.status === TaskStatus.InProgress).length === 0
  ) {
...
function updateTasksCount(tasks: Task[]) {
  ...
  const todoCountElement = Helper.getElementById<HTMLSpanElement>("todo-count");
  const inProgressCountElement =
    Helper.getElementById<HTMLSpanElement>("progress-count");
  const doneCountElement =
    Helper.getElementById<HTMLSpanElement>("completed-count");
...
function handleEditTask(taskId: string) {
  ...
  const saveBtn = document.getElementById("addTaskBtn") as HTMLButtonElement;
...
function updateTaskStatus(taskId: string | null, newStatus: TaskStatus) {
  if (!taskId) return;
  const tasks = JSON.parse(localStorage.getItem("tasks") || "[]") as Task[];
  const taskIndex = tasks.findIndex((t: Task) => t.id === taskId);
